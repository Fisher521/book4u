...
function buildSystemPrompt(opts: {
  ...
  const profileBlock = profileLines.length
    ? `\n────────── 这个 ta 的画像 ──────────\n\n${profileLines.join('\n\n')}\n\n**"定锚池"只是校准样本**——告诉你"这种 MBTI 喜欢这种味道的作家"。
真正起作用的是"味道"那行（沉静/内敛/抒情/智识游戏…）。
推荐时**用味道做筛子，从全世界找作家**——池内那 5-8 人只是味觉参照，不是选项菜单。`
    : '';
...
- **aligned**：MBTI 池正常用
- **mild_drift**：池里找极端那一头（INFJ 偏激时找 Anne Carson 这种带刀的）
- **strong_conflict**：弃 baseline 借对面 MBTI 池，**mood_summary 必须显式承认"今晚不像平时的你"**
